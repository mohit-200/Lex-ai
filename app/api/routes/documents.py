...
from app.models.db import Document, RiskAnalysis
...
async def upload_document(
    ...
    Triggers: parsing → chunking → embedding → indexing in Qdrant.
    Then kicks off background risk analysis via Celery.
    """
    # Validate file type
    ...
    # Parse PDF into clause-aware chunks
    ...
    # Create pending risk analysis record
    risk = RiskAnalysis(document_id=document_id, status="pending")
    db.add(risk)
    await db.commit()

    # Index in Qdrant (embed + store)
    ...
    # Kick off background risk analysis
